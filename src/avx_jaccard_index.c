...
void avx2_jaccard_index_lookup(const uint64_t* dataA, const uint64_t* dataB, size_t n, uint64_t* j_union, uint64_t* j_inter) {
    size_t i = 0;
    ...
    __m256i acc_union = _mm256_setzero_si256();
    __m256i acc_inter = _mm256_setzero_si256();

#define ITER { \
        const __m256i vecA = _mm256_loadu_si256((const __m256i*)(dataA + i)); \
        const __m256i vecB = _mm256_loadu_si256((const __m256i*)(dataB + i)); \
        const __m256i vec_union = _mm256_or_si256(vecA, vecB); \
        const __m256i vec_inter = _mm256_and_si256(vecA, vecB); \
        const __m256i lo_union  = _mm256_and_si256(vec_union, low_mask); \
        const __m256i lo_inter  = _mm256_and_si256(vec_inter, low_mask); \
        const __m256i hi_union  = _mm256_and_si256(_mm256_srli_epi16(vec_union, 4), low_mask); \
        const __m256i hi_inter  = _mm256_and_si256(_mm256_srli_epi16(vec_inter, 4), low_mask); \
        const __m256i popcnt1_union = _mm256_shuffle_epi8(lookup, lo_union); \
        const __m256i popcnt1_inter = _mm256_shuffle_epi8(lookup, lo_inter); \
        const __m256i popcnt2_union = _mm256_shuffle_epi8(lookup, hi_union); \
        const __m256i popcnt2_inter = _mm256_shuffle_epi8(lookup, hi_inter); \
        local_union = _mm256_add_epi8(local_union, popcnt1_union); \
        local_union = _mm256_add_epi8(local_union, popcnt2_union); \
        local_inter = _mm256_add_epi8(local_inter, popcnt1_inter); \
        local_inter = _mm256_add_epi8(local_inter, popcnt2_inter); \
        i += 4; \
    }
    ...
    while (i + 8*4 <= n) {
        local_union = _mm256_setzero_si256();
        local_inter = _mm256_setzero_si256();
        ITER ITER ITER ITER
        ITER ITER ITER ITER
        acc_union = _mm256_add_epi64(acc_union, _mm256_sad_epu8(local_union, _mm256_setzero_si256()));
        acc_inter = _mm256_add_epi64(acc_inter, _mm256_sad_epu8(local_inter, _mm256_setzero_si256()));
    }
...
#undef ITER
    ...
    *j_union += (uint64_t)(_mm256_extract_epi64(acc_union, 0));
    *j_union += (uint64_t)(_mm256_extract_epi64(acc_union, 1));
    *j_union += (uint64_t)(_mm256_extract_epi64(acc_union, 2));
    *j_union += (uint64_t)(_mm256_extract_epi64(acc_union, 3));

    *j_inter += (uint64_t)(_mm256_extract_epi64(acc_inter, 0));
    *j_inter += (uint64_t)(_mm256_extract_epi64(acc_inter, 1));
    *j_inter += (uint64_t)(_mm256_extract_epi64(acc_inter, 2));
    *j_inter += (uint64_t)(_mm256_extract_epi64(acc_inter, 3));

    for (/**/; i < n; i++) {
        *j_union += _mm_popcnt_u64(dataA[i] | dataB[i]);
        *j_inter += _mm_popcnt_u64(dataA[i] & dataB[i]);
    }
}
...
void avx2_jaccard_index_harley_seal(const uint64_t* dataA, const uint64_t* dataB, size_t size, uint64_t* j_union, uint64_t* j_inter) {
  const unsigned int wordspervector = sizeof(__m256i) / sizeof(uint64_t);
  const unsigned int minvit = 16 * wordspervector;
  if(size >= minvit) {
    jaccard_index((const __m256i*) dataB, (const __m256i*) dataA, size / wordspervector, j_union, j_inter);
