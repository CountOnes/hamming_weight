...
int avx2_lookup2_bitset64_weight(const uint64_t* data, size_t n) {
    size_t i = 0;
    const __m256i lookup = _mm256_setr_epi8(
        /* 0 */ 0, /* 1 */ 1, /* 2 */ 1, /* 3 */ 2,
        /* 4 */ 1, /* 5 */ 2, /* 6 */ 2, /* 7 */ 3,
...
#define ITER { \
        ...
        const __m256i popcnt1 = _mm256_shuffle_epi8(lookup, lo); \
        const __m256i popcnt2 = _mm256_shuffle_epi8(lookup, hi); \
        local1 = _mm256_add_epi8(local1, popcnt1); \
        local2 = _mm256_add_epi8(local2, popcnt2); \
        i += 4; \
    }

    while (i + 8*4 <= n) {
        __m256i local1 = _mm256_setzero_si256();
        __m256i local2 = _mm256_setzero_si256();
        ITER ITER ITER ITER
        ITER ITER ITER ITER
        __m256i local = _mm256_add_epi8(local1, local2); 
        acc = _mm256_add_epi64(acc, _mm256_sad_epu8(local, _mm256_setzero_si256()));
    }
