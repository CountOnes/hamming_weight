...
int sse_twocounters_bitset64_weight(const uint64_t* data, size_t n) {
    size_t i = 0;
    const __m128i lookup = _mm_setr_epi8(
        /* 0 */ 0, /* 1 */ 1, /* 2 */ 1, /* 3 */ 2,
        /* 4 */ 1, /* 5 */ 2, /* 6 */ 2, /* 7 */ 3,
...
#define ITER { \
        ...
        const __m128i popcnt1 = _mm_shuffle_epi8(lookup, lo); \
        const __m128i popcnt2 = _mm_shuffle_epi8(lookup, hi); \
        local1 = _mm_add_epi8(local1, popcnt1); \
        local2 = _mm_add_epi8(local2, popcnt2); \
        i += 2; \
    }

    while (i + 8*2 <= n) {
        __m128i local1 = _mm_setzero_si128();
        __m128i local2 = _mm_setzero_si128();

        ITER ITER ITER ITER
        ITER ITER ITER ITER
        __m128i local = _mm_add_epi8(local1,local2);
