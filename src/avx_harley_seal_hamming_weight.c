...
// Kim Walish's version of popcount_pshufb
static __m256i popcountwalisch(__m256i v) {
...
static uint64_t popcntwalisch(const __m256i* data, const uint64_t size) {
  __m256i total     = _mm256_setzero_si256();
  __m256i ones      = _mm256_setzero_si256();
  __m256i twos      = _mm256_setzero_si256();
  __m256i fours     = _mm256_setzero_si256();
  ...
  for(; i < limit; i += 16) {
    ...
    total = _mm256_add_epi64(total, popcountwalisch(sixteens));
  }

  total = _mm256_slli_epi64(total, 4);     // * 16
  total = _mm256_add_epi64(total, _mm256_slli_epi64(popcountwalisch(eights), 3)); // += 8 * ...
  total = _mm256_add_epi64(total, _mm256_slli_epi64(popcountwalisch(fours),  2)); // += 4 * ...
  total = _mm256_add_epi64(total, _mm256_slli_epi64(popcountwalisch(twos),   1)); // += 2 * ...
  total = _mm256_add_epi64(total, popcountwalisch(ones));
  for(; i < size; i++)
    total = _mm256_add_epi64(total, popcountwalisch(_mm256_lddqu_si256(data + i)));
...
int avx2_harley_seal_walisch_bitset64_weight(const uint64_t * data, size_t size) {
  const unsigned int wordspervector = sizeof(__m256i) / sizeof(uint64_t);
  const unsigned int minvit = 16 * wordspervector;
  int total;
  if(size >= minvit) {
    total = popcntwalisch((const __m256i*) data, size / wordspervector);
    for (size_t i = size - size % wordspervector; i < size; i++) {
      total += _mm_popcnt_u64(data[i]);
    }
    return total;
